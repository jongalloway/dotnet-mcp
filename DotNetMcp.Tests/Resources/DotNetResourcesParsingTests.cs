...
public class DotNetResourcesParsingTests
{
    ...
    public void ParseSdkListOutput_ParsesAndSorts_WithStableAfterPrereleaseForSameBaseVersion()
    {
        ...
        Assert.Equal(Path.Join("C:\\Program Files\\dotnet\\sdk", "10.0.101"), sdks[0].Path);
        Assert.Equal(Path.Join("C:\\Program Files\\dotnet\\sdk", "11.0.100-preview.1"), sdks[1].Path);
        Assert.Equal(Path.Join("C:\\Program Files\\dotnet\\sdk", "11.0.100"), sdks[2].Path);
    }
    ...
    public void ParseRuntimeListOutput_ParsesValidLines_IgnoresInvalid()
    {
        ...
        Assert.Equal(Path.Join("C:\\Program Files\\dotnet\\shared\\Microsoft.NETCore.App", "10.0.1"), runtimes[0].Path);

        Assert.Equal("Microsoft.AspNetCore.App", runtimes[1].Name);
        Assert.Equal("10.0.1", runtimes[1].Version);
        Assert.Equal(Path.Join("C:\\Program Files\\dotnet\\shared\\Microsoft.AspNetCore.App", "10.0.1"), runtimes[1].Path);
    }
